...
- проверки сообщений коммитов на соответствие спецификации [Conventional Commits](https://www.conventionalcommits.org/ru/v1.0.0)
...
- Установите последнюю версию компилятора языка Go с официального сайта https://go.dev/dl.
...
- Репозиторий должен использовать Git.
  **ВАЖНО:** используйте `git push --tags`. Опция `--tags` также пушит
  тэги, которые используются утилитой для определения текущей версии.
...
- Иметь осмысленный заголовок в формате Conventional Commits (тип[область_видимости]: короткий глагол), отражающий суть изменения.
...
Почему это важно:
- Для *bump-version* утилитарный анализ истории основывается на типах коммитов (feat/fix/BREAKING CHANGE). Если один коммит содержит несколько разных типов изменений, утилита может неверно определить тип релиза или отнести изменение к неверной категории в CHANGELOG.
...
- Правила повышения версии на основе истории коммитов (как применяет *bump-version*):
  ...
  - **MINOR** — повышается, если нет breaking changes, но есть хотя бы один коммит типа `feat`. Это добавление новой обратно-совместимой функциональности.
  - **PATCH** — повышается, если нет breaking changes и feat-коммитов, но есть коммиты типа `fix`. Это исправления багов.
  - Никаких изменений — если нет коммитов типов feat/fix/BREAKING CHANGE (утилита завершается без релиза).

- Дополнительные нюансы:
  - Коммиты типов `perf`, `refactor`, `docs`, `chore` и т.п. по умолчанию не повышают версию, но могут включаться в changelog в соответствующие разделы. При желании, коммиты с такими типами можно включить в CHANGELOG (см. [настройки утилиты](#настройки-утилиты)).
  ...
  - Префикс `v` в тэге Git (например, `v1.2.3`) — опционален и настраивается через поле `"versionTagFormat"` в конфигурационном файле (см. [настройки утилиты](#настройки-утилиты)). Семантика версии от этого не меняется.
...
Для полноты рекомендуется прочитать спецификацию: https://www.conventionalcommits.org/ru/v1.0.0
...
```
  ...
  всех коммитов с последнего релиза есть команда `bump-version lint`. Также, если `ignoreInvalidCommits` не
...
```bash
bump-version команда -config other-bump-version-config.cfg
```

- Добавить конфигурационный файл `bump-version.cfg` в корень проекта:
```bash
...
Файл конфигурации утилиты должен лежать в корне проекта и иметь имя `bump-version.cfg`.
Если файл не найден, то используется конфигурация по умолчанию.
...
- `versionFilenames` — имена файлов-версий, разделенные запятой.
...
- `allowedCommitKinds` — разрешенные типы коммитов, разделенные запятой.
- `bumpVersionCommit` — формат сообщения коммита, связанного с выпуском версии.
- `shouldPushToOrigin` — нужно ли пушить основную ветку при выпуске версии.
...
- Файл-версии (например, `VERSION`) — содержит строку с текущей версией.
...
- **Вопрос:** Что произойдёт, если в истории нет коммитов, соответствующих Conventional Commits?
  - **Ответ:** Утилита выводит ошибку или предупреждение о проигнорировании таких коммитов. Вам прийдется сделать `git rebase -i` для исправления коммитов.

- **Вопрос:** Я выпустил новую версию слишком рано. Что делать?
  - **Ответ:** Выполните команду `bump-version cancel`.
